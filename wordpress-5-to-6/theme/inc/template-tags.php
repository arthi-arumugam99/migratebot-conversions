...
if ( ! function_exists( '_tw_post_thumbnail' ) ) :
	...
	function _tw_post_thumbnail() {
		...
		if ( is_singular() ) :
			...
			<figure>
				<?php
				// MIGRATED: WP 6.3 — fetchpriority="high" added for above-the-fold singular post thumbnail (LCP image).
				the_post_thumbnail(
					'post-thumbnail',
					array(
						'fetchpriority' => 'high', // MIGRATED: WP 6.3 — marks hero/featured image as high priority for LCP performance
						'decoding'      => 'sync',  // MIGRATED: WP 6.x — use sync decoding for the LCP image to avoid layout shifts
					)
				);
				?>
			...
			<figure>
				<a href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
					<?php
					// MIGRATED: WP 6.x — decoding="async" added for non-critical archive/listing thumbnails.
					the_post_thumbnail(
						'post-thumbnail',
						array(
							'decoding' => 'async', // MIGRATED: WP 6.x — async decoding for non-critical images improves rendering performance
						)
					);
					?>
				</a>
...
if ( ! function_exists( '_tw_comment_avatar' ) ) :
	...
	function _tw_get_user_avatar_markup( $id_or_email = null ) {
		...
		// MIGRATED: WP 6.x — decoding="async" added for non-critical avatar images.
		return sprintf(
			'<div class="vcard">%s</div>',
			get_avatar(
				$id_or_email,
				_tw_get_avatar_size(),
				'',
				'',
				array(
					'extra_attr' => 'decoding="async"', // MIGRATED: WP 6.x — async decoding for avatar images improves rendering performance
				)
			)
		);
	}
