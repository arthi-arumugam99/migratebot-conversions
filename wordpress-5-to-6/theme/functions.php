...
if ( ! function_exists( '_tw_setup' ) ) :
	...
	function _tw_setup() {
		...
		// MIGRATED: WP 6.x — add_editor_style() applies to both the block editor and the classic editor; no change needed here.
 ...
 *
 * This script handles applying Tailwind Typography classes inside the block
 * editor canvas. It replaces the classic-editor approach of injecting classes
 * via `tiny_mce_before_init` (see `_tw_tinymce_add_class` below).
 */
 ...
 *
 * MIGRATED: WP 6.x — The block editor is now the default editor. The
 * Tailwind Typography classes are injected into the block editor canvas by
 * `_tw_enqueue_block_editor_script()` / `block-editor.js` instead.
 *
 * This filter is kept for backward compatibility with:
 *   - Sites that have the Classic Editor plugin active.
 *   - Third-party plugins that render a TinyMCE instance (e.g. Advanced
 *     Custom Fields classic text areas).
 *
 * TODO: Manual review — If the Classic Editor plugin is no longer required
 * and no third-party plugin depends on TinyMCE, this filter and its callback
 * can be removed entirely. Verify by auditing all `wp_editor()` calls and
 * any ACF/meta-box fields that use the "wysiwyg" type before deleting.
 *
 * @param array $settings TinyMCE settings.
 * @return array
 * @deprecated WP 6.x — Use block editor canvas class injection via block-editor.js instead.
 */
function _tw_tinymce_add_class( $settings ) {
	$settings['body_class'] = _TW_TYPOGRAPHY_CLASSES;
	return $settings;
}
// MIGRATED: WP 6.x — tiny_mce_before_init kept for Classic Editor / ACF TinyMCE backward compatibility only.
